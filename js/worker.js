...
const WorkerOnboarding = memo(({ worker, myOps, onDone }) => {
  ...
  const Step1 = () => h('div', { style: { textAlign: 'center' } },
    ...
    h('div', { style: { fontSize: 13, color: 'var(--muted)', marginBottom: 24, lineHeight: 1.6 } },
    ...
    h('button', { style: { background: 'none', border: 'none', fontSize: 12, color: 'var(--muted)', cursor: 'pointer', width: '100%', padding: '6px' }, onClick: onDone }, 'Пропустить')
  );

  const Step2 = () => h('div', null,
    h('div', { style: { fontSize: 14, color: 'var(--muted)', marginBottom: 12 } },
      myOps.length > 0 ? `Ваши задания на сегодня (${myOps.length}):` : 'Ваши задания'
    ),
    myOps.length > 0
      ? myOps.slice(0, 3).map((op, i) => h('div', { key: op.id, style: { ...{ background: 'var(--card-solid,#fff)', border: '0.5px solid rgba(0,0,0,0.1)', borderRadius: 10, padding: '10px 14px', marginBottom: 8 } } },
      ...
      : h('div', { style: { background: 'var(--card-2)', borderRadius: 10, padding: '20px', textAlign: 'center', color: 'var(--muted)', fontSize: 13, marginBottom: 12 } },
...
const WorkerNotificationsBlock = memo(({ workerId, data }) => {
  ...
  }[sev] || { bg: 'var(--card-2)', border: '#ccc', icon: '#888', text: '#333' });
...
const WorkerOutputChart = memo(({ workerId, data }) => {
  ...
  return h('div', { style: { ...S.card, marginBottom: 16 } },
    ...
    // Легенда
    h('div', { style:{ display:'flex', gap:12, marginTop:6, fontSize:11, color:'var(--muted)' } },
...
const WorkerSizesBlock = memo(({ workerId, data, onUpdate, addToast }) => {
  ...
  return h('div', { style: { ...S.card, marginBottom: 16 } },
    ...
    saved.updatedAt && !editing && h('div', { style: { fontSize:10, color:'var(--muted)', marginTop:8 } },
...
const WorkerOpsHistoryBlock = memo(({ workerId, data }) => {
  ...
  return h('div', { style: { ...S.card, marginBottom: 16 } },
    ...
    // Список
    filtered.length === 0
      ? h('div', { style:{ fontSize:13, color:'var(--muted)', textAlign:'center', padding:'16px 0' } },
...
const WorkerSalaryBlock = memo(({ workerId, data }) => {
  ...
  return h('div', { style: { ...S.card, marginBottom: 16 } },
    ...
    // Нет ставки
    noRate && h('div', { style:{ background:'var(--card-2)', borderRadius:8, padding:'12px 14px', fontSize:13, color:'var(--muted)', textAlign:'center' } },
    ...
    !noRate && expanded && payType === 'hourly' && h('div', null,
      salaryData.dayRows.length === 0
        ? h('div', { style:{ fontSize:13, color:'var(--muted)', textAlign:'center', padding:'12px 0' } }, 'Нет явок в этом месяце')
        : h('div', { style:{ display:'flex', flexDirection:'column', gap:3 } },
            salaryData.dayRows.map(({ d, h: hrs, earn }) =>
              h('div', { key:d, style:{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'5px 10px', borderRadius:6, background:'var(--card-2)', fontSize:13 } },
                h('span', { style:{ color:'var(--muted)' } }, `${String(d).padStart(2,'0')}.${String(viewMonth+1).padStart(2,'0')}`),
                h('span', null, `${hrs}ч`),
            ...
            h('div', { style:{ display:'flex', justifyContent:'space-between', padding:'7px 10px', borderTop:`0.5px solid rgba(0,0,0,0.08)`, marginTop:4, fontSize:13, fontWeight:500 } },
              h('span', { style:{ color:'var(--muted)' } }, 'Итого'),
        ...
        : h('div', { style:{ display:'flex', flexDirection:'column', gap:3 } },
            salaryData.pieceRows.map(({ id, number, product, qty, earn, date, breakdown }) =>
              h('div', { key:id, style:{ padding:'6px 10px', borderRadius:6, background:'var(--card-2)', fontSize:13 } },
                ...
                breakdown && h('div', { style:{ display:'flex', gap:8, marginTop:4, fontSize:11, color:'var(--muted)', flexWrap:'wrap' } },
...
const WorkerHoursBlock = memo(({ workerId, data }) => {
  ...
  return h('div', { style: { ...S.card, marginBottom: 16 } },
    ...
    h('div', { style: { display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:12 } },
      h('div', { style:{ background:'var(--card-2)', borderRadius:8, padding:'8px 10px', textAlign:'center' } },
        h('div', { style:{ fontSize:22, fontWeight:500, color:AM } }, `${monthData.totH}ч`),
        h('div', { style:{ fontSize:10, color:'var(--muted)', textTransform:'uppercase', marginTop:2 } }, 'Отработано')
      ),
      h('div', { style:{ background:'var(--card-2)', borderRadius:8, padding:'8px 10px', textAlign:'center' } },
        h('div', { style:{ fontSize:22, fontWeight:500, color:GN } }, monthData.totDays),
        h('div', { style:{ fontSize:10, color:'var(--muted)', textTransform:'uppercase', marginTop:2 } }, 'Дней явок')
      ),
      h('div', { style:{ background:'var(--card-2)', borderRadius:8, padding:'8px 10px', textAlign:'center' } },
        h('div', { style:{ fontSize:22, fontWeight:500 } }, monthData.totOps),
        h('div', { style:{ fontSize:10, color:'var(--muted)', textTransform:'uppercase', marginTop:2 } }, 'Операций')
      )
    ),
...
const WorkerScreen = memo(({ data, workerId, sectionId, onUpdate, initialOpId, addToast }) => {
  ...
  // Отдельная модалка для оплачиваемых допрасценок (стыки, вальцовка и т.п.) — цена из data.extraWorks
  const [showAddExtra, setShowAddExtra] = useState(false);
  const [extraForm, setExtraForm] = useState({ orderId: '', catKey: '', param: '', qty: 1, executors: [], comment: '' });
  ...
  // Оплачиваемая допработа (стыки, вальцовка и т.п.) с ценой из каталога extraWorks.
  // Создаёт op со status:'pending_approval' — мастер утверждает в master.js.
  // Цена ЗАМОРАЖИВАЕТСЯ в op.extraAmount на момент создания (изменения каталога позже
  // не влияют на уже созданные записи — это защита истории от ретроактивных пересчётов).
  const selectedExtraCat = (data.extraWorks || []).find(c => c.key === extraForm.catKey);
  const extraPriceMatch = selectedExtraCat && extraForm.param !== ''
    ? findExtraWorkPrice(data, extraForm.catKey, extraForm.param) : null;

  const addExtraWorkOp = useCallback(async () => {
    if (!extraForm.orderId) { addToast('Выберите заказ', 'error'); return; }
    if (!extraForm.catKey) { addToast('Выберите категорию допработы', 'error'); return; }
    if (extraForm.param === '' || !isFinite(Number(extraForm.param))) { addToast('Укажите параметр (число)', 'error'); return; }
    const cat = (data.extraWorks || []).find(c => c.key === extraForm.catKey);
    if (!cat) { addToast('Категория не найдена', 'error'); return; }
    const match = findExtraWorkPrice(data, extraForm.catKey, extraForm.param);
    if (!match) { addToast(`Для ${extraForm.param}${cat.paramUnit || ''} в категории «${cat.name}» цена не найдена. Обратитесь к HR или мастеру.`, 'error'); return; }
    const qty = Math.max(1, Number(extraForm.qty) || 1);
    const executors = (extraForm.executors && extraForm.executors.length > 0) ? extraForm.executors : [workerId];
    // Гарантируем, что сам работник в списке исполнителей (защита от кривого UI)
    const workerIds = executors.includes(workerId) ? executors : [workerId, ...executors];
    const order = data.orders.find(o => o.id === extraForm.orderId);
    const totalAmount = match.price * qty;
    const newOp = {
      id: uid(),
      name: `${cat.name} — ${extraForm.param}${cat.paramUnit || ''}${qty > 1 ? ' × ' + qty : ''}`,
      orderId: extraForm.orderId,
      workerIds,
      status: 'pending_approval',   // ждём согласования мастера
      createdAt: now(),
      sectionId: null, archived: false,
      isExtraWork: true,
      extraKey: cat.key,
      extraParam: Number(extraForm.param),
      extraQty: qty,
      extraPrice: match.price,
      extraAmount: totalAmount,      // общая сумма (до деления между исполнителями), заморожена
      addedByWorker: workerId,
      comment: extraForm.comment.trim() ? `[${worker?.name || 'Рабочий'}]: ${extraForm.comment.trim()}` : undefined,
    };
    let d = { ...data, ops: [...data.ops, newOp] };
    d = logAction(d, 'worker_add_extra_work', {
      opId: newOp.id, orderId: newOp.orderId, orderNumber: order?.number,
  ...
  }, [data, extraForm, workerId, worker, onUpdate, addToast]);
  ...
  return h('div', { style: { maxWidth: 440, margin: '0 auto', padding: '0 12px 80px' } },
    ...
    workerTab === 'tasks' && h('div', null,
          ...
          return h('div', { key: op.id, className: 'op-card-anim', style: {
            ...
            h('div', { style: { marginBottom: 10 } },
              ...
              h('div', { style: { display: 'flex', gap: 8, alignItems: 'center', marginTop: 4 } },
                ...
                !depsComplete && h('span', { style: { fontSize: 11, color: 'var(--muted)' } }, '🔒 ожидает предыдущие')
              )
            ),
            op.comment && h('div', { style: { fontSize: 12, color: 'var(--fg-muted)', background: 'var(--card-2)', padding: '6px 10px', borderRadius: 6, marginBottom: 10 } }, `💬 ${op.comment}`),
            (() => {
            ...
            op.status === 'pending' && canStart
              ...
              : op.status === 'pending' && !depsComplete
                ? h('div', { style: { textAlign: 'center', padding: '12px 0', color: 'var(--muted)', fontSize: 12 } }, 'Ожидание предыдущих этапов')
      ...
      myOps.length === 0 && h('div', { className: 'empty-state', style: { marginTop: 32 } },
        ...
        h('div', { style: { fontSize: 13, color: 'var(--muted)', marginBottom: 16 } }, 'Новые задания появятся здесь')
      ),

      // Кнопка «Записать доп. работы» — доступна когда нет активной операции
      h('div', { style: { marginTop: myOps.length > 0 ? 8 : 0, marginBottom: 8, display:'flex', gap: 6, flexDirection: 'column' } },
        ...
        // Кнопка появляется только если в системе заведены допрасценки — иначе смысла нет
        (data.extraWorks || []).length > 0 && h('button', {
          style: { ...gbtn({ width: '100%', padding: '12px 16px', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }), borderStyle: 'dashed', borderWidth: '1.5px', background: 'rgba(217,166,58,0.06)', borderColor: 'rgba(217,166,58,0.5)' },
          onClick: () => setShowAddExtra(true)
        }, '+ Доп. расценка (со сдельной оплатой)')
      ),

      // Модал допрасценки (оплачиваемая) — привязка к заказу обязательна, цена из каталога
      showAddExtra && h('div', {
        ...
        h('div', { style: { background: 'var(--card-solid,#fff)', borderRadius: 14, padding: 20, width: 'min(460px, calc(100vw - 32px))', maxHeight: '85vh', overflowY: 'auto' } },
          h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 } },
            h('div', { style: { fontSize: 16, fontWeight: 500 } }, '💰 Доп. расценка'),
            h('button', { style: { background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: 'var(--muted)', padding: 4, minHeight: 'auto' }, onClick: () => setShowAddExtra(false) }, '×')
          ),
          h('div', { style: { fontSize: 12, color: 'var(--muted)', marginBottom: 16, lineHeight: 1.5 } },
            'Оплачиваемая допработа (стыки, вальцовка и т.п.). После сохранения запись уйдёт мастеру на согласование, после чего попадёт в вашу зарплату.'
          ),

          // Заказ (обязательно)
          h('div', { style: { marginBottom: 12 } },
            h('label', { style: S.lbl }, 'Заказ *'),
            h('select', { style: { ...S.inp, width:'100%' }, value: extraForm.orderId,
              onChange: e => setExtraForm(p => ({ ...p, orderId: e.target.value })) },
              h('option', { value: '' }, '— выберите заказ —'),
              availableOrders.map(o => h('option', { key: o.id, value: o.id },
                `${o.number}${o.boilerType && o.powerKw ? ` · ${o.boilerType} ${o.powerKw}кВт` : ''}${o.customer ? ` · ${o.customer}` : ''}`
              ))
            )
          ),
          ...
          h('div', { style: { marginBottom: 12 } },
            ...
            h('div', { style: { display:'flex', flexWrap:'wrap', gap: 6 } },
              (data.extraWorks || []).map(cat => h('button', {
                key: cat.id,
                style: extraForm.catKey === cat.key
                  ? abtn({ fontSize: 12, padding:'8px 12px' })
                  : gbtn({ fontSize: 12, padding:'8px 12px' }),
                onClick: () => setExtraForm(p => ({ ...p, catKey: cat.key, param: '' }))
              }, cat.name))
            )
          ),

          // Параметр + предпросмотр цены
          selectedExtraCat && h('div', { style: { marginBottom: 12 } },
            h('label', { style: S.lbl }, `${selectedExtraCat.paramLabel} *`),
            h('div', { style: { display:'flex', gap: 8, alignItems:'center' } },
              h('input', { type:'number', style: { ...S.inp, flex: 1 },
                placeholder: `Значение в ${selectedExtraCat.paramUnit || ''}`,
                value: extraForm.param,
                onChange: e => setExtraForm(p => ({ ...p, param: e.target.value })) }),
              h('span', { style: { fontSize: 12, color:'var(--muted)' } }, selectedExtraCat.paramUnit || '')
            ),
            // Показ найденной цены / ошибки
            extraForm.param !== '' && (extraPriceMatch
              ? h('div', { style: { marginTop: 6, padding: '6px 10px', background:'#eaffea', borderRadius: 6, fontSize: 12, color:'#0a7' } },
                  `✓ Цена: ${extraPriceMatch.price.toLocaleString('ru-RU')} ₽ (диапазон ${extraPriceMatch.tier.min}${extraPriceMatch.tier.min !== extraPriceMatch.tier.max ? '-' + extraPriceMatch.tier.max : ''}${selectedExtraCat.paramUnit || ''})`
                )
              : h('div', { style: { marginTop: 6, padding: '6px 10px', background:'#fce8e8', borderRadius: 6, fontSize: 12, color:'#a00' } },
                  `⚠ Для ${extraForm.param}${selectedExtraCat.paramUnit || ''} цена не задана. Обратитесь к HR.`
                ))
          ),

          // Количество
          selectedExtraCat && h('div', { style: { marginBottom: 12 } },
            h('label', { style: S.lbl }, 'Количество'),
            h('input', { type:'number', min: 1, style: { ...S.inp, width: 100 }, value: extraForm.qty,
              onChange: e => setExtraForm(p => ({ ...p, qty: e.target.value })) })
          ),
          ...
          selectedExtraCat && h('div', { style: { marginBottom: 12 } },
            ...
            h('div', { style: { maxHeight: 160, overflowY:'auto', border:'0.5px solid rgba(0,0,0,0.1)', borderRadius: 6, background:'#fafafa', padding: 6 } },
              (data.workers || [])
                ...
                .map(w => {
                  const isSelf = w.id === workerId;
                  const isChecked = isSelf || (extraForm.executors || []).includes(w.id);
                  return h('label', { key: w.id, style: {
                    display:'flex', alignItems:'center', gap: 8, padding:'6px 8px', borderRadius: 4,
                    cursor: isSelf ? 'default' : 'pointer',
                    background: isChecked ? 'rgba(217,166,58,0.1)' : 'transparent',
                    opacity: isSelf ? 0.7 : 1
                  } },
                    h('input', { type:'checkbox', checked: isChecked, disabled: isSelf,
                      onChange: e => {
                        setExtraForm(p => {
                          const cur = new Set(p.executors || []);
                          if (e.target.checked) cur.add(w.id); else cur.delete(w.id);
                          return { ...p, executors: Array.from(cur) };
                        });
                      }
                    }),
                    h('span', { style: { fontSize: 13 } }, w.name + (isSelf ? ' (вы)' : ''))
                  );
                })
            )
          ),
          ...
          selectedExtraCat && h('div', { style: { marginBottom: 12 } },
            ...
            h('input', { style: { ...S.inp, width:'100%' }, placeholder:'Что именно делали...',
              value: extraForm.comment, onChange: e => setExtraForm(p => ({ ...p, comment: e.target.value })) })
          ),
          ...
          extraPriceMatch && (() => {
            ...
            return h('div', { style: { marginBottom: 12, padding: 10, background:'#f5f5f2', borderRadius: 6 } },
              ...
              execCount > 1 && h('div', { style: { fontSize: 13, fontWeight: 500 } },
                `На ${execCount} чел. по ${perPerson.toLocaleString('ru-RU')} ₽`),
              execCount === 1 && h('div', { style: { fontSize: 13, fontWeight: 500 } },
                `Вам: ${total.toLocaleString('ru-RU')} ₽`)
            );
          })(),

          h('div', { style: { display:'flex', gap: 8 } },
            h('button', { style: abtn({ flex: 1, padding: '10px 16px' }),
              onClick: addExtraWorkOp,
              disabled: !extraForm.orderId || !extraForm.catKey || !extraPriceMatch
            }, '✓ Отправить мастеру'),
            h('button', { style: gbtn({ padding: '10px 16px' }), onClick: () => setShowAddExtra(false) }, 'Отмена')
          )
        )
      ),
      ...
      showAddOp && h('div', {
        ...
        h('div', { style: { background: 'var(--card-solid,#fff)', borderRadius: 14, padding: 20, width: 'min(420px, calc(100vw - 32px))', maxHeight: '85vh', overflowY: 'auto' } },
          h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 } },
            h('div', { style: { fontSize: 16, fontWeight: 500 } }, '📝 Дополнительные работы'),
            h('button', { style: { background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: 'var(--muted)', padding: 4, minHeight: 'auto' }, onClick: () => setShowAddOp(false) }, '×')
          ),
          h('div', { style: { fontSize: 12, color: 'var(--muted)', marginBottom: 16, lineHeight: 1.5 } },
              ...
              return h('div', { key: op.id, style: { ...S.card, marginBottom: 6, padding: 10, cursor: 'pointer', borderLeft: op.status === 'defect' ? `3px solid ${RD}` : op.status === 'on_check' ? `3px solid ${BL}` : `3px solid ${GN}` }, onClick: () => setHistoryOp(isExp ? null : op.id) },
                ...
                isExp && h('div', { style: { marginTop: 8, paddingTop: 8, borderTop: '0.5px solid rgba(0,0,0,0.06)', fontSize: 12 } },
                  h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 12px' } },
                    h('div', null, h('span', { style: { color: 'var(--muted)' } }, 'Начало: '), op.startedAt ? new Date(op.startedAt).toLocaleTimeString() : '—'),
                    h('div', null, h('span', { style: { color: 'var(--muted)' } }, 'Длит.: '), dur),
                    op.plannedHours && h('div', null, h('span', { style: { color: 'var(--muted)' } }, 'План: '), `${op.plannedHours}ч`)
                  ),
                  op.defectNote && h('div', { style: { marginTop: 4, color: RD } }, h('span', { style: { color: 'var(--muted)' } }, 'Дефект: '), op.defectNote),
                  op.comment && h('div', { style: { marginTop: 4 } }, h('span', { style: { color: 'var(--muted)' } }, 'Комментарий: '), op.comment)
                )
              );
            })
          )
      ...
      showDowntimeModal && h('div', { role: 'dialog', 'aria-modal': 'true', style: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60 } },
        h('div', { className: 'modal-content', style: { background: 'var(--card-solid,#fff)', borderRadius: 14, padding: 20, width: 'min(380px, calc(100vw - 24px))', maxHeight: '85vh', overflowY: 'auto' } },
          ...
          data.equipment.length > 0 && h('div', { style: { marginBottom: 12 } },
            h('div', { style: { fontSize: 12, color: 'var(--muted)', marginBottom: 6 } }, 'Оборудование:'),
    ...
    workerTab === 'profile' && h('div', null,
      ...
      (() => {
        ...
        return h('div', { style: { marginBottom: 16, padding: '12px 14px', background: 'var(--card-solid,#fff)', borderRadius: 12, border: '0.5px solid rgba(0,0,0,0.06)' } },
          ...
          h('div', { style: { fontSize: 11, color: 'var(--muted)' } }, msg)
        );
      })(),
      ...
      showThanksHistory && myStats.recentThanks.length > 0 && h('div', { style: { ...S.card, marginBottom: 16, padding: 12 } },
        myStats.recentThanks.map((t, i) => h('div', { key: i, style: { display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '0.5px solid rgba(0,0,0,0.05)', fontSize: 13 } },
          h('div', null, h('span', { style: { fontWeight: 500 } }, `🤝 ${t.from}`), t.note && h('span', { style: { color: 'var(--muted)', marginLeft: 6 } }, t.note)),
          h('div', { style: { fontSize: 11, color: 'var(--muted)' } }, new Date(t.ts).toLocaleDateString())
        ))
      ),
      ...
      h('div', { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
        Object.entries(ACHIEVEMENTS).slice(0, showAchievements ? undefined : 6).map(([id, a]) => {
          ...
          return h('div', { key: id, style: { display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 10, background: earned ? AM3 : 'var(--card-2)', border: `0.5px solid ${earned ? AM4 : 'rgba(0,0,0,0.06)'}` } },
            ...
            h('div', { style: { flex: 1 } },
              h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
                ...
                earned ? h('span', { style: { fontSize: 12, color: GN } }, '✓') : pr && h('span', { style: { fontSize: 11, color: 'var(--muted)' } }, `${pr.cur}/${pr.target}`)
              ),
