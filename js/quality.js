...
const ControllerScreen = memo(({ data, onUpdate, addToast, onOrderClick, onWorkerClick }) => {
  ...
  return h('div', null,
    viewOrderId && h(OrderCardModal, { orderId: viewOrderId, data, onUpdate, onClose: () => setViewOrderId(null), canEdit: false, allowRouteSheet: true }),
    h(TabBar, { tabs: [
  ...
  return h('div', null,
    ...
    showForm && h('div', { style: { ...S.card, marginBottom: 12, border: `0.5px solid ${AM}` } },
      ...
      h('div', { className: 'form-row', style: { display: 'flex', gap: 8, flexWrap: 'wrap' } },
        h('div', { style: { flex: 2, minWidth: 160 } }, h('label', { style: S.lbl }, 'Заказ'), h('select', { style: { ...S.inp, width: '100%' }, value: form.orderId, onChange: e => setForm(p => ({ ...p, orderId: e.target.value })) }, h('option', { value: '' }, '— заказ —'), data.orders.filter(o => !o.archived && !o.isParentOrder).map(o => h('option', { key: o.id, value: o.id }, `${o.number} — ${o.product}`)))),
