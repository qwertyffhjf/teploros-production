...
const AiAnalyst = memo(({ data, period, allData }) => {
  ...
  const hasKey = !!(data.settings?.groqApiKey || data.settings?.geminiApiKey || data.settings?.aiApiKey);
...
РЕКЛАМАЦИИ за период: ${recl.length}`;
  ...
  const analyze = async (customQuestion) => {
    const apiKey = data.settings?.groqApiKey || data.settings?.geminiApiKey || data.settings?.aiApiKey;
    if (!apiKey) { setError('API ключ не задан. Добавьте Groq / Gemini / Claude API Key в Система → Настройки.'); return; }
...
Пиши кратко и по делу, как опытный производственник.`;
    ...
    try {
      // 1) Groq — бесплатный, быстрый, OpenAI-совместимый эндпоинт.
      // Пробуем первым: у Gemini бывают гео-блокировки ("User location is not supported"),
      // у Groq на практике их не встречалось.
      const groqKey = data.settings?.groqApiKey;
      if (groqKey) {
  ...
  return h('div', { style: { ...S.card, marginBottom: 12, border: `0.5px solid ${AM4}` } },
    ...
    h('div', { style: { display:'flex', alignItems:'center', gap:8, marginBottom: expanded ? 12 : 0 } },
      ...
      h('div', { style: { flex:1 } },
        ...
        h('div', { style: { fontSize:11, color:'var(--muted)' } },
          data.settings?.groqApiKey ? 'Groq (Llama 3.3 70B) · готов к работе'
    ...
    expanded && h('div', null,
      ...
      error && h('div', { style:{ padding:'10px 12px', background: RD3, borderRadius:8, fontSize:12, color: RD2, marginTop:8 } },
        '⚠ ', error,
        error.includes('API ключ') && h('span', null, ' → Система → Настройки → Groq / Gemini / Claude API Key')
      ),
